...
second ring. A node's **size is its summed sub storage capacity** (what the planet can hold, fixed
across the match — *not* its momentary ship count; the reserve/storage node is excluded). Nodes
show **per-side present ship counts** — the player's and enemy's `Structure::ship_count`, each in
its faction colour and stacked when both are present (incoming inter-planet fleets are not counted
until they land) — the planet **name**, a production pip on owned planets, and an `AUTO` ring/tag on
automated ones. The planet's **reserve-node garrison is drawn as dots** on a ring inside the node,
...
- **Sub-structures**: disk + owner ring + production progress ring + **per-side present counts**
  (`idle_count_at(i, Player/Enemy)`, each in its faction colour, home-based, excluding incoming).
...
### Pacing
Fixed-tick world stepping with render interpolation: `BASE_TICKS_PER_SEC = 2.5` at 1x (deliberately
slow so a human can read the board), decisions every `DECISION_INTERVAL = 5` ticks, capped at
`MAX_TICKS_PER_FRAME = 8` ticks per rendered frame. The top bar carries **Pause / x1 / x3** buttons
(`SPEED_IDX_1X = 1`, `SPEED_IDX_3X = 4`); the keyboard still works — **`P`** pauses, **`-` / `+`**
(or `[` / `]`) step the full `0.5 / 1 / 1.5 / 2 / 3 / 4 / 6×` ladder. Death flashes live for
`KILL_FX_TTL = 0.35 s`.
...
- mouse-wheel **with a sub selected (interior)** — resize that sub's orbit ring (does not zoom)
...
- top-bar **Pause / x1 / x3** buttons, or `P` — pause / resume
- `-` / `+` (or `[` / `]`) — slower / faster
