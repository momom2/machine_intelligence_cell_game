    ...
    fn build(
        ...
        let infos = (0..st.subs.len())
            .map(|s| {
                ...
                // Sum engaging ships of every other real seat (all are foes).
                let enemy_ships = [Faction::Player, Faction::Ai(0), Faction::Ai(1)]
                    .into_iter()
                    .filter(|&f| f != seat)
                    .map(|f| engaging_count(st, params, s, f))
                    .sum::<usize>() as u32;
...
impl<'a> PositionView for Layer1View<'a> {
    ...
    fn enemy_incoming(&self, id: usize) -> u32 {
        ...
        match self.proj {
            ...
            Some((proj, p)) => [Faction::Player, Faction::Ai(0), Faction::Ai(1)]
                .into_iter()
                .filter(|&f| f != self.seat)
                .map(|f| proj.incoming_present_at(p, id, f))
                .sum(),
            None => 0,
        }
    }
