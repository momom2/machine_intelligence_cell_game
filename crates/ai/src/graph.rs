...
//!
//! Both need the same two primitives: the **shortest-path length** between two structs (to
//! pick a "nearest" objective) and the **first hop** along that path (because a
//! `world::FleetOrder` is only valid between lane-adjacent structs, so a multi-lane move must
//! be routed one lane at a time). They are plain Dijkstra over lane lengths on the small
//! struct graph (an O(V^2) scan — fine here and order-stable, lowest id breaking ties).

use world::{StructId, World};

/// Per-struct Dijkstra result from a single source: `dist[v]` (sum of lane lengths, `INFINITY`
...
fn sssp(world: &World, from: StructId) -> Sssp {
...
/// `from == to` is `Some(0.0)`.
pub fn path_len(world: &World, from: StructId, to: StructId) -> Option<f32> {
    if from == to {
        return Some(0.0);
    }
    if from >= world.structs.len() || to >= world.structs.len() {
        return None;
    }
...
/// First struct on a shortest path from `from` to `to` (the lane-adjacent struct a
/// `world::FleetOrder` should target to move toward `to`), or `None` if `to` is unreachable or
/// `from == to`.
pub fn next_hop(world: &World, from: StructId, to: StructId) -> Option<StructId> {
    if from == to || from >= world.structs.len() || to >= world.structs.len() {
        return None;
    }
    let s = sssp(world, from);
