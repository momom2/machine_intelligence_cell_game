...
impl World {
    ...
    ///   straight-line ETA the sim/projection use (`ceil((dist-tolerance)/ship_speed)`);
    ...
    ///
    /// Free-for-all: every real faction other than `seat` is counted as a foe. Deterministic: a pure
    /// function of the world state (positions, fleets) in f32 with a single `ceil`, so identical inputs
    /// give an identical influx and `state_hash` replay stays bit-identical.
    pub fn sub_influx_for(
        ...
        const EPS: f32 = 1e-6;
        let st = &self.planets[planet].structure;
        let n = st.subs.len();
        ...
        for sh in &st.ships {
            ...
            if sh.faction == seat {
                ...
                let eta = now + (eff / sp.ship_speed.max(EPS)).ceil() as u64;
    ...
    pub fn outcome(&self) -> WorldOutcome {
        ...
        // The two AI seats (`Enemy`, `Enemy2`) are aggregated into the binary "enemy" slot (they may
        // also have whittled each other down — which simply helps the player).
