...
fn check_determinism(level: &Level) -> bool {
    ...
    let replay = |level: &Level| -> (Vec<u64>, world::WorldOutcome) {
        ...
        let enemy = AiController::from_roster(Faction::Ai(0), level.primary_enemy());
...
fn not_auto_lost(level: &Level) -> (bool, String) {
    ...
    for &seed in &VALIDATION_SEEDS {
        ...
        let o = if layer1_micro {
            run_layer1_concentration_proxy(&mut w, &params, &wp, level.primary_enemy(), level.horizon)
        } else {
            match_on(&mut w, &params, &wp, Roster::GreedyLocal, level.primary_enemy(), level.horizon)
        };
...
fn run_layer1_concentration_proxy(
    ...
    let enemy_ctrl = AiController::from_roster(Faction::Ai(0), enemy);
