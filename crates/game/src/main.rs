...
/// A seat's AI driver. Most rosters are the stateless [`AiController`]; the **Simple** roster is the
/// stateful [`ai::SimpleController`] (it carries a per-planet departure ledger across ticks), so the
/// enemy collection holds this dispatch enum and is stepped with `&mut`.
enum SeatController {
    Stateless(AiController),
    Simple(ai::SimpleController),
}

impl SeatController {
    /// Build the right driver for `seat` from a roster entry: the stateful Simple for
    /// [`ai::Roster::SimpleColonize`], else the stateless controller.
    fn from_roster(seat: Faction, r: ai::Roster) -> SeatController {
        match r {
            ai::Roster::SimpleColonize => SeatController::Simple(ai::SimpleController::new(seat)),
    ...
    fn decide_and_apply(&mut self, world: &mut World, sim: &SimParams, wp: &WorldParams) -> (usize, usize) {
        match self {
...
impl Game {
    fn new(level: Level, seed: u64, auto: bool, force_view: Option<ViewTarget>, scale: f64) -> Game {
        let (world, wp) = build_scaled(&level, seed, scale);
    ...
    /// Colour for the `i`-th AI seat: its roster's kind-colour, in a distinct shade per seat so
    /// same-kind rivals are told apart (seat 0 = base shade, seats 1+ = the alt shade). Indexes
    ...
    /// True once **every** enemy seat is [`Game::seat_finished`] — the player's win condition in a
    /// free-for-all (a single-enemy level's `Enemy2` is trivially finished, so this reduces to
    /// `seat_finished(Enemy)`).
    fn all_enemies_finished(&self) -> bool {
        self.seat_finished(Faction::Ai(0)) && self.seat_finished(Faction::Ai(1))
    }
    ...
    fn update(&mut self, dt: f64) {
        ...
        self.render_alpha = (self.tick_accum / FIXED_DT).clamp(0.0, 1.0) as f32;
    ...
    fn spawn_kill_fx(&mut self, now: f64) {
        let r2 = self.sim.engagement_radius * self.sim.engagement_radius;
        ...
        for (p, planet) in self.world.planets.iter().enumerate() {
            ...
            for id in 0..prev.len() {
                ...
                let sh = &st.ships[id];
                // A *random* living enemy still in range becomes the line source (matches combat's
                // random-target model); none in range ⇒ just the cross (e.g. a soft-cap death).
                let in_range: Vec<layer1::Vec2> = st
                    .ships
                    .iter()
                    .filter(|o| {
                        o.alive
                            && o.faction.is_real()
                            && o.faction != sh.faction
                            && sh.pos.dist_sq(o.pos) <= r2
                    })
                    .map(|o| o.pos)
                    .collect();
                let from = if in_range.is_empty() {
                    None
                } else {
                    Some(in_range[macroquad::rand::gen_range(0, in_range.len())])
                };
...
fn handle_in_level_input(game: &mut Game) {
    ...
    if is_mouse_button_released(MouseButton::Left) {
        let start = game.drag_start.take();
        if game.box_active {
...
fn draw_interior(game: &Game, p: PlanetId, cam: &Camera, alpha: f32) {
    ...
    for (i, s) in st.subs.iter().enumerate() {
        ...
        // produces nothing, so it gets none.)
        if !is_storage {
...
fn draw_ships_interior(game: &Game, p: PlanetId, st: &layer1::Structure, cam: &Camera, ox: f32, oy: f32, alpha: f32) {
    ...
    if on_screen > SHIP_DENSITY_THRESHOLD {
    ...
    SHIP_MESH.with(|m| {
        ...
        for id in 0..st.ships.len() {
            ...
            if sh.drift_remaining > 0 {
                col.a *= 0.8 * (sh.drift_remaining as f32 / layer1::sim::DRIFT_TICKS as f32);
            }
