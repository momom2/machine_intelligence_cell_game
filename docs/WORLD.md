...
> `StructAggregate` fold **all** non-player seats into one combined-enemy slot (`World::total_foreign_*`),
...
> **Refreshed for `feat/counter`** — the inter-struct plumbing now routes through each struct's
...
(`LAYER1_SIM.md`). Every **struct** *is* a real `layer1::Structure` (its own sub-structures and
discrete ships, all sharing one `SimParams`). Layer 2 adds only two things on top:

- **lanes** — edges between structs, and
- **inter-struct fleets** — clumps of ships in transit from one struct to another.

It is **not** a second combat model. `World::step` steps every struct's `Structure` (which
fights, captures, and produces exactly as before) and moves ships between structs along lanes.
When a fleet arrives, it **injects** its ships into the destination struct's `Structure`,
...
beyond `layer1` (all randomness stays inside each struct's seeded PRNG).

---

## The model

### Structures — one Layer-1 `Structure` each
...
pub struct Structure {
    ...
    pub pos: layer1::Vec2,            // the struct's position on the LAYER-2 map
    pub name: String,
}
```

The struct *is* its `structure`: all of its sub-structures, ships, production, combat, and
capture happen inside it under the shared `layer1::SimParams`. `pos` is where the struct sits on
the **Layer-2** map — a separate space from the *intra*-struct coordinates inside `structure`
(used to draw the struct and to give lanes a direction). Structures are referenced by
**`StructId = usize`** (the index into `World::structs`).

### Lanes — Layer-2 edges

A **`Lane { a: StructId, b: StructId, length: f32 }`** is an **undirected** edge between two
structs. `length` (in Layer-2 map units) sets how long a fleet takes to cross. `World` keeps a
deterministic **adjacency** index (`World::neighbors(p)`), rebuilt as lanes are added.

### Inter-struct fleets — ships in transit
...
pub struct InterFleet {
    ...
    pub from: StructId,          // source struct (ships were pulled from here)
    pub to: StructId,            // destination struct (ships injected here on arrival)
...
It **mirrors Layer-1's undock-then-transit movement** so inter-struct travel feels consistent
with intra-struct movement: the fleet first burns `undock_remaining` ticks leaving the source
...
### `WorldParams` — the inter-struct dials

Intra-struct behaviour is governed by `layer1::SimParams`; the **inter-struct** layer has its
...
| `keep_floor` | **2** | Per-sub idle **garrison floor** kept on the source struct when a fleet launches: no owned sub is drawn below this many idle ships, so a struct never exports itself empty. |

### `FleetOrder` — the inter-struct atomic action

```rust
pub struct FleetOrder { pub from: StructId, pub to: StructId, pub fraction: FractionBucket }
```
...
struct — it is **faction-scoped**, so it only ever moves the acting seat's ships and never drags
...
- `faction` is `Neutral`,
- the source struct has no exportable idle surplus for `faction` (everything is below the
...
The pull uses the Layer-1 helper `Structure::take_idle_ships_structwide(faction, fraction,
keep_floor)` (see below; the fraction-slider path uses the sibling
`take_idle_ships_structwide_fraction`). Both delegate to one core, `export_idle_structwide`, which
departs **reserve-first**:

- **If the struct's reserve / patrol-zone node (the `storage_sub`) holds idle ships of `faction`**,
...
- **Only when the reserve is empty** (or the structure has no reserve) does the pull fall back to
  the producing subs: the bucket applies to the faction's **total idle ships across the struct**,
...
destination struct's `Structure` as `faction`, spawned **idle** (via `Structure::spawn_ship`),
...
1. **If the destination struct has a reserve / patrol-zone node** (the `storage_sub`), the fleet
   lands **into the reserve** — the universal inter-struct entry/exit point. Every campaign struct
...
2. **Otherwise (a bare structure with no reserve)**, fall back to the lane-facing `entry_sub`,
   ...
   - Compute `dir` = the unit vector **from the destination struct toward the source struct on the
...
this tick's struct steps, so freshly landed ships first fight on the **next** tick — the same
...
1. **Structures** — step every struct's `Structure` in ascending `StructId` order (each does its
...
4. **tick** += 1 (every struct's own `tick` advances in lock-step with the world tick).

---

## The Layer-2 aggregate (the lens datum)

`World::struct_aggregate(p) -> StructAggregate` is the **lens data** per struct — what the
renderer draws, and what the strategic AI and the greedy export rule read instead of peering at
every sub-structure. It is computed from the struct's `Structure` plus the in-transit fleets
headed to it; it adds no state.

```rust
pub struct StructAggregate {
    pub owner: StructOwner,        // Owned(faction) | Contested | Neutral
    pub player_ships: usize,       // living Player ships GARRISONED on the struct
    pub enemy_ships: usize,        // living Enemy ships garrisoned on the struct
    pub player_incoming: u32,      // Player ships in fleets whose `to` is this struct
...
contribute to ownership, to `fully_owned_uncontested`, or to the owner rule above; a struct is
...
**Ship counts** include both **garrisoned** ships (in the struct's `Structure`) and ships
**currently arriving** (`*_incoming`). `StructAggregate::ships_of(faction)` returns the sum.
...
fleet inbound to a struct you fully own does not, on its own, make that struct contested or
block you from exporting from it — until it lands.)

**Exportable flag:** `StructAggregate::fully_owned_uncontested(faction)` is `true` iff `faction`
owns **every** sub on the struct (no enemy *and* no neutral sub remains) **and no enemy ship is
present** (garrisoned). This is the precondition a later phase uses to decide a struct may
...
## Resistance / soft-cap read signals (struct-scope wrappers)

The resistance / denial / soft-cap overhaul (`LAYER1_SIM.md`, `AUTOMATA_DESIGN.md` §1) added
per-sub / per-structure reads to `layer1::Structure`. `World` re-exports three at **struct scope**
so the strategic AI never reaches into a struct's `Structure`. All are pure, deterministic reads
that add no state and draw no randomness; out-of-range `p` yields the zero value.

- **`struct_total_resistance_vs(p, seat) -> f32`** — total foreign capture resistance on struct `p`
...
- **`parked_count(p, seat) -> u32`** — parked ships of `seat` on struct `p` (living ships in the
  struct's `Structure` — idle or intra-structure transit). Inter-struct fleets to/from `p` live in
  `World::fleets` and are **not** counted (they are soft-cap-exempt). Wraps `Structure::parked_count`.
- **`soft_cap(p, seat, sp) -> u32`** — the soft cap for `seat` on struct `p`: `softcap_free` plus the
  **sum of per-sub capacities** of the subs `seat` owns there (numerically `softcap_free +
  softcap_per_sub · owned_subs` today). When `parked_count` exceeds it, the struct's `Structure`
  bleeds the overflow with `sqrt` attrition. Wraps `Structure::soft_cap`.

These are the struct-level signals the **parked** automata (Defend's reserve/spend logic, the
...
It answers, for each sub of each struct:
...
> *and* inter-struct `InterFleet`s, each counted from its **arrival tick**), and folding in the
...
structure moving ships *and* inter-struct fleets at once.
...
> the struct's reserve node when one exists (`storage_sub.or_else(entry_sub)`). On every campaign
> struct (all of which have a reserve) the projection therefore schedules the arrival into a
...
touches no struct's seeded `Rng`, and draws **no randomness**, so calling it never perturbs
...
since-deleted `proj-bench` binary, on a representative six-struct world — 21 subs, ~100 garrisoned
...
The result is a `Projection` holding one `SubFate` per `(struct, sub)` (O(1) lookup), the scheduled
...
- `capture_eta(struct, sub) -> Option<u64>` — absolute tick of the first projected owner change on
  the *current* plan, or `None` if it does not flip within the horizon.
- `capture_eta_if(struct, sub, extra, arriving_in_ticks, reinforce) -> Option<u64>` — the flip tick
...
- `marginal_ticks_saved(target_struct, target_sub, from_position) -> u64` — the value, **in ticks
  saved on the capture**, of one more ship sent from `from_position` (a sub on the same struct, whose
...
**Force-sizing query.**
- `force_for_efficiency(struct, sub, desired_ratio) -> Option<u32>` — the **smallest** attacking
...
- `current_owner(struct, sub) -> Faction` — owner at call time.
- `sub_resistance(struct, sub) -> (f32, f32)` — `(current, max)` resistance at call time.
- `present_now(struct, sub) -> (u32, u32)` — the `(player, enemy)` idle ships seeded as initial
  presence (these cannot be read off `SubFate` alone).

**Per-sub & struct roll-ups.**
- `sub_fate(struct, sub) -> &SubFate` — the full per-sub forecast: `current_owner`,
  ...
  (`>= base_tick`).
- `sub_capture(struct, sub) -> Option<(Faction, u64)>` — who captures/frees the sub first, and when.
- `struct_capture(struct) -> Option<(Faction, u64)>` — struct-level roll-up: a struct "flips to" a
...
- `incoming_present_at(struct, sub, faction) -> u32` — how many of `faction`'s scheduled in-transit
...
- `returning_owner_force(struct, sub) -> u32` — the in-flight arrivals of the sub's **current
  owner** within the horizon (Attack sizes a heal-outlasting hold from this).
- `eta_to_present_for(struct, sub, faction) -> Option<u64>` — the tick by which `faction` is first
  present at the sub (now if already present, else its earliest scheduled arrival).
- `struct_first_fall(struct, seat) -> Option<(SubId, u64)>` — the first `seat`-owned sub projected to
  flip to the enemy and when (Defend's "reinforce the sub that falls first").

Layer-2 strategies read `struct_capture` / `struct_first_fall`; Layer-1 / interior strategies read
...
**all** structs and **all** in-transit fleets:

- A faction is **eliminated** when it owns **no sub on any struct** *and* has **no ships
...
`World::total_ships(faction)` counts garrisoned ships across every struct **plus** every ship in
a fleet of that faction; `World::total_subs(faction)` sums owned subs across structs — and since it
is built on `Structure::sub_count`, the per-struct **reserve nodes are excluded** from the
...
- **All randomness stays inside each struct's `Structure`**, behind its own seeded PRNG. The
  inter-struct layer draws **none** — launching, transiting, and resolving arrivals are all
  RNG-free. (Injection's per-ship spawn jitter is drawn from the *destination struct's* own RNG,
...
- The **new Layer-1 export helpers draw no randomness** (see below), so extracting a fleet from
  a struct does not perturb that struct's combat rolls.
- **`World::step` has a fixed iteration order** (structs by `StructId`, then fleets in vector
  order, then arrivals in vector order).
- **`World::state_hash()`** folds, with the same inline FNV-1a construction `layer1` uses (floats
  by bit pattern, exact): the world `tick`, then **every struct's `Structure::state_hash()`**
  (which already includes that struct's full sim state *and* RNG position) plus its map `pos`,
...
reruns**, and an extra order **diverges** the hash. Cloning a `World` clones every struct's RNG,
...
To launch a fleet, the world must **remove idle ships from a source struct** and re-spawn them
...
- **`take_idle_ships(&mut self, sub: SubId, faction: Faction, n: usize) -> usize`** — remove up
  ...
  `n == 0` removes nothing.
- **`take_idle_ships_structwide(&mut self, faction, fraction: FractionBucket, keep_floor: usize)
  -> usize`** (and its continuous sibling `take_idle_ships_structwide_fraction(faction, frac,
  ...
  delegate to the private `export_idle_structwide` core. Returns the true count removed (≤ the
...
```rust
use world::{
    World, Structure, Lane, InterFleet, WorldParams,        // model
    FleetOrder, StructId,                                 // orders / ids
    StructAggregate, StructOwner, WorldOutcome,           // lens / outcome
...
// Build a world: each struct wraps a layer1::Structure placed on the Layer-2 map.
let mut w = World::new();
let a = w.add_struct(Structure::new(structure_a, Vec2::new(0.0, 0.0), "A"));
let b = w.add_struct(Structure::new(structure_b, Vec2::new(40.0, 0.0), "B"));
w.add_lane(a, b, 40.0);                       // undirected edge, length 40

let params = SimParams::default();            // intra-struct dials (shared by all structs)
let wp = WorldParams::default();              // inter-struct dials
...
// Advance the whole world one tick (steps every struct, moves fleets, resolves arrivals).
w.step(&params, &wp);

// Lens / queries for the renderer and the strategic AI:
let agg: StructAggregate = w.struct_aggregate(b);
let exportable = agg.fully_owned_uncontested(Faction::Player);
let neighbors: &[StructId] = w.neighbors(a);
let connected = w.are_connected(a, b);
let outcome: WorldOutcome = w.outcome();

// Resistance / soft-cap struct-scope signals the strategic AI reads:
let foreign_res = w.struct_total_resistance_vs(b, Faction::Player); // grind to fully own b
...
let plan = proj.struct_capture(b);                                 // struct-level roll-up
...
cargo test -p world           # the world tests (multi-struct step, arrival+capture,
...
- **multi-struct step** — every struct's Layer-1 sim advances under one `World::step`.
- **inter-struct fleet** — a fleet launched from A arrives at B and the injected ships **capture
  a neutral sub** there; plus a **beachhead** variant (no foothold ⇒ lands at the sub facing the
  source lane).
- **junk-order safety** — unconnected/same-struct/out-of-range/neutral/no-surplus orders are
  no-ops.
- **`StructAggregate`** — neutral, owned (+ exportable), contested, and "incoming counts but does
  not flip the owner."
- **determinism** — two identical runs match per-tick and final `state_hash`; an extra order
  diverges; a clone replays identically.
- **AI-free 2-struct smoke** — both sides push periodic fleets and the world runs to a horizon
