...
- a **`ring_frac`** (default `DEFAULT_RING_FRAC = 0.667`): the fraction of the radius at which idle
  ships orbit (the WYSIWYG ring; see below). Real sim state — folded into `state_hash`.
...
**`is_storage(sub)`**. It is a big circle (radius = enclosing radius of the real subs · 1.35) that
**encloses** the producing subs and is the **universal inter-planet entry/exit point**: inter-planet
fleets arrive into it and depart from it reserve-first (falling back to subs only while it is empty).
It mints **nothing** (`production == 0`) and has a huge `storage_capacity = STORAGE_RESERVE_CAP =
6000`. It is **capturable** but confers no production, so it is **excluded** from `sub_count`,
`is_eliminated`, `total_subs`, level `spec_for`, and territory tallies everywhere. Its initial owner
is the structure's current majority sub-owner. (Full design + wiring points: `memory/struct-storage.md`.)
...
64-bit FNV-1a fingerprint of the entire state for exact comparison. Per sub it now folds in `pos`,
`radius`, `owner`, `production_timer`, **`resistance` and `max_resistance`** (so a divergent grind is
detected), **`ring_frac`**, **`production`**, and **`produce_cursor`**; per ship it folds in
`faction`, `pos`, `alive`, `home`, `target`, `aim`, and **`angle`** (the orbit phase is game state);
then the tick and the RNG position. (`undock_remaining` is *not* hashed.) Cloning a `Structure`
clones its RNG, so a clone replays identically (useful for renderer replay/prediction).
...
|---|---|---|---|
| Engagement radius `R` | `engagement_radius` | **7.0** | A ship is engaged when a living enemy is within this **fixed** distance (independent of sub radius). |
...
| Default ring fraction | `DEFAULT_RING_FRAC` (module const) | **0.667** | Idle-ship orbit radius as a fraction of sub radius. Player-adjustable in the GUI. |
...
| `sub_presence(sub)` | `SubPresence { player, enemy, owner }` | Both seats' **radius**-present counts plus the owner (counts movers / passers-through too). A spatial read — **not** the capture driver; for that use `capture_presence`. |
| `single_present_faction(sub)` | `Option<(Faction, u32)>` | The lone **radius**-present seat and its count, or `None` for the frozen case (zero or both present). Spatial; the home-based grind discriminant is `capture_present_faction`. |
| `capture_presence(sub)` | `SubPresence { player, enemy, owner }` | The **home-based** present counts (idle ships with `home == sub`) the grind actually feeds to `capture_step` — the WYSIWYG erosion truth a renderer/host should read. |
| `capture_present_faction(sub)` | `Option<(Faction, u32)>` | The lone **home-based** present seat (the grind discriminant), or `None` when zero or both are present — what `resolve_resistance` keys off, so "being captured by whom?" matches the sim. |
